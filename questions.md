...
### 10. Работа с текстом: vim, grep, sed, less/more, man
...
**Календарные выражения для таймеров:**
```bash
# Примеры OnCalendar
...
**Команда ss для современного мониторинга:**
```bash
# Базовый мониторинг
...
**nload - мониторинг пропускной способности интерфейсов:**
```bash
# Установка
...
**chrony - современная альтернатива NTP:**
```bash
# Установка chrony
...
ntpdate -q pool.ntp.org              # проверить разницу без установки

# Анализ логов NTP
tail -f /var/log/ntp.log
journalctl -u ntp -f

# Скрипт мониторинга времени
#!/bin/bash
while true; do
    echo "=== $(date) ==="
    echo "Системное время: $(date)"
    echo "Аппаратное время: $(hwclock)"
    echo "Статус NTP:"
    ntpstat
    echo "---"
    sleep 60
...
```bash
# Проверка сетевых подключений к NTP серверам
...
sudo ufw allow out 123/udp           # разрешить исходящий NTP трафик
```
...
### 45. Управление пакетами в различных дистрибутивах
...
### 46. Сборка ПО из исходных кодов
...
### 47. Управление службами и автозапуск
...
### 47. Работа с репозиториями

**Управление репозиториями APT (Debian/Ubuntu):**
```bash
# Файлы репозиториев
/etc/apt/sources.list                 # основной файл репозиториев
/etc/apt/sources.list.d/              # дополнительные репозитории
...
# main - основные пакеты с поддержкой
# restricted - пакеты с ограниченной лицензией
# universe - пакеты сообщества
# multiverse - пакеты с ограничениями распространения
```
...
add-apt-repository -y ppa:deadsnakes/ppa    # Python репозиторий
...
apt-key list                          # список ключей
apt-key add keyfile                   # добавить ключ из файла
apt-key del KEYID                     # удалить ключ

# Современный способ (apt-key deprecated)
curl -fsSL https://example.com/key.gpg | sudo gpg --dearmor -o /etc/apt/keyrings/example.gpg
echo "deb [signed-by=/etc/apt/keyrings/example.gpg] https://example.com/repo focal main" | sudo tee /etc/apt/sources.list.d/example.list
```

**YUM/DNF репозитории (RHEL/CentOS/Fedora):**
```bash
# Файлы конфигурации репозиториев
/etc/yum.repos.d/                     # директория с .repo файлами
...
dnf repolist --enabled                # только включённые
dnf config-manager --add-repo URL     # добавить репозиторий
dnf config-manager --enable repo_name # включить репозиторий
dnf config-manager --disable repo_name # отключить репозиторий

# YUM (RHEL 7, CentOS 7)
yum repolist                          # список репозиториев
yum-config-manager --add-repo URL     # добавить репозиторий
yum-config-manager --enable repo_name # включить репозиторий
```

**Популярные дополнительные репозитории:**

**EPEL (Extra Packages for Enterprise Linux):**
```bash
# Установка EPEL
dnf install epel-release              # Fedora, RHEL 8+
yum install epel-release              # RHEL 7, CentOS 7

# Или вручную
dnf install https://dl.fedoraproject.org/pub/epel/epel-release-latest-8.noarch.rpm
```

**RPM Fusion (мультимедиа пакеты для Fedora):**
```bash
...
```bash
# APT
apt clean                             # очистить кэш пакетов
apt autoclean                         # очистить устаревший кэш
apt autoremove                        # удалить неиспользуемые зависимости

# DNF/YUM
dnf clean all                         # очистить весь кэш
dnf autoremove                        # удалить неиспользуемые пакеты
yum clean all                         # для YUM
```

**Приоритеты репозиториев:**
```bash
# APT - файл /etc/apt/preferences
Package: *
Pin: release a=focal
Pin-Priority: 500
...
# DNF - в файле .repo
[repo-name]
priority=1                            # 1 = высший приоритет, 99 = низший
```

**Зеркала и прокси:**
```bash
# APT прокси (/etc/apt/apt.conf.d/01proxy)
Acquire::http::Proxy "http://proxy:8080";
Acquire::https::Proxy "https://proxy:8080";

# Выбор зеркала для APT
sed -i 's|http://archive.ubuntu.com|http://mirror.local.com|g' /etc/apt/sources.list

# DNF прокси (/etc/dnf/dnf.conf)
...
### 48. Подключение по SSH, копирование файлов по SSH
...
### 50. Настройка SSH-клиента и сервера
...
### 51. Криптография DSA/ECDSA, её применение
...
### 56. Основные концепции Docker, его инфраструктура
...
### 59. Различие образов alpine/slim/buster
...
### 60. Жизненный цикл Docker-контейнера
...
### 61. Команды для работы с Docker-контейнером
...
### 65. Статическая и динамическая компиляция
...
### 67. Синтаксис docker-compose.yaml
...
### 70. Протокол HTTP/1.1 (опционально HTTP/2.0, HTTP/3.0)
...
### 71. Взаимодействие с веб-серверами
...
### 81. Производительность системы и оптимизация
...
### 82. Мониторинг производительности веб-приложений
...
### 83. Оптимизация производительности
...
### 84. Linux kernel: модули, системные вызовы
